...
describe 'when viewing the orders' do
  before(:each) do
    User.create(email: 'yourdad123@aol.com', first_name: 'your', last_name: 'dad', password: '123', password_confirmation: '123', role: 'borrower')
    login_as_borrower
  end
    ...
    let(:address) { Address.create(street: '1 Blake St', city: 'Denver', state: 'CO', zip: '80000') }
      ...
      address
		...
		xit 'can remove items from an order' do
			...
			expect(current_path).to eq(edit_borrower_order_path(order))
			click_link('Remove')
			expect(current_path).to eq(edit_borrower_order_path(order))
